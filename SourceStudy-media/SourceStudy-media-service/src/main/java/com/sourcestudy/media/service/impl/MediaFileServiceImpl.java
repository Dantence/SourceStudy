...
import com.sourcestudy.media.service.MediaFileService;
...
public class MediaFileServiceImpl implements MediaFileService {
    ...
    public UploadFileResultDto uploadFile(Long companyId, UploadFileParamsDto uploadFileParamsDto, byte[] bytes, String folder, String objectName) {
        ...
        try {
            ...
            e.printStackTrace();
            SourceStudyException.cast("上传过程中出错");
        }
        return null;
    ...
    public void addMediaFilesToMinIO(byte[] bytes, String bucket, String objectName) {
        ...
        try {
            ...
            e.printStackTrace();
            SourceStudyException.cast("上传文件到文件系统出错");
        }
    }
     ...
     * @param bucket              桶
     * @param objectName          对象名称
     * @return com.sourcestudy.media.model.po.MediaFiles
    ...
    public MediaFiles addMediaFilesToDb(Long companyId, String fileMd5, UploadFileParamsDto uploadFileParamsDto, String bucket, String objectName) {
        ...
        if (mediaFiles == null) {
            ...
            if (insert < 0) {
                SourceStudyException.cast("保存文件信息失败");
            }
    ...
    public RestResponse uploadChunk(String fileMd5, int chunk, byte[] bytes) {
        ...
        try {
            ...
            ex.printStackTrace();
            SourceStudyException.cast("上传过程出错请重试");
        }
    ...
    private File[] checkChunkStatus(String fileMd5, int chunkTotal) {
        ...
        for (int i = 0; i < chunkTotal; i++) {
            ...
            try {
                ...
                e.printStackTrace();
                SourceStudyException.cast("下载分块时创建临时文件出错");
            }
    ...
    public File downloadFileFromMinIO(File file, String bucket, String objectName) {
        ...
        try {
            ...
            try {
                ...
                SourceStudyException.cast("下载文件" + objectName + "出错");
            }
        } catch (Exception e) {
            e.printStackTrace();
            SourceStudyException.cast("文件不存在" + objectName);
        } finally {
            if (fileInputStream != null) {
                try {
    ...
    public RestResponse mergechunks(Long companyId, String fileMd5, int chunkTotal, UploadFileParamsDto uploadFileParamsDto) {
        ...
        try {
            ...
            SourceStudyException.cast("合并文件过程中创建临时文件出错");
        }
        ...
        try {
            ...
            try (RandomAccessFile raf_write = new RandomAccessFile(mergeFile, "rw");) {
                ...
                e.printStackTrace();
                SourceStudyException.cast("合并文件过程中出错");
            }
            ...
            try (InputStream mergeFileInputStream = new FileInputStream(mergeFile);) {
                ...
                if (!fileMd5.equalsIgnoreCase(newFileMd5)) {
                    //校验失败
                    SourceStudyException.cast("合并文件校验失败");
                }
                log.debug("合并文件校验通过{}", mergeFile.getAbsolutePath());
            } catch (Exception e) {
                e.printStackTrace();
                //校验失败
                SourceStudyException.cast("合并文件校验异常");
            }
    ...
    public MediaFiles getFileById(String id) {
        ...
        if (mediaFiles == null) {
            SourceStudyException.cast("文件不存在");
        }
        if (StringUtils.isEmpty(mediaFiles.getUrl())) {
            SourceStudyException.cast("文件还未处理，请稍后再试");
        }
        return mediaFiles;
